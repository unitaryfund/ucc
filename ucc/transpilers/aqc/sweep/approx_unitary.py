...
from .typing import UNITARY_LAYER


def hst_fidelity(
    u: NDArray[np.complex128], target: NDArray[np.complex128], n: int
) -> float:
    """Calculate Hilbert-Schmidt test to find the fidelity between
    unitaries `u` and `target`.

    Args:
        u (NDArray[np.complex128]): Unitary matrix to be compared with target.
        target (NDArray[np.complex128]): Target unitary matrix.
        n (int): Number of qubits used by `u` and `target`.

    Returns:
        (float): The fidelity between `u` and `target`.
    """
    return 1 / 2 ** (2 * n) * np.abs(np.trace(u.conj().T @ target)) ** 2


class UnitarySweepApproximator:
    """Initialize the UnitarySweepApproximator class.

    Notes
    -----
    This algorithm performs sweeps over unitary layers representing
    an ansatz to be trained to match the target unitary to improve
    ...
    the same task as here but for states under Oall approach.
    ...
    def get_tensor_network_from_unitary_layers(
        ...
        Args:
            num_qubits (int): The number of qubits used by target unitary.
        ...
        tensor_network = qtn.TensorNetwork(circuit.get_uni())
    ...
    def sweep_unitary_layers(
        target_mpo: qtn.MatrixProductOperator,
        ...
        Args:
            target_mpo (qtn.MatrixProductOperator): The target unitary represented as a MPO.
        ...
        """
        target_mpo_adjoint = target_mpo.conj()

        current_tn = qtn.MatrixProductOperator.from_dense(
            tensor_network.to_dense(tensor_network.outer_inds())
        )

        for i, gate in enumerate(reversed(tensor_network.tensors)):
            ...
            # For the first tensor, the environment tensor
            # is simply what remains of the MPO
            if i == len(tensor_network.tensors) - 1:
                environment_tensor = target_mpo_adjoint
            else:
                ...
                # the updated tensor to MPO
                current_tn = current_tn @ gate.conj()
                environment_tensor: qtn.TensorNetwork = (
                    target_mpo_adjoint @ current_tn
                )

            u, _, vh = np.linalg.svd(
                environment_tensor.to_dense((left_inds), (right_inds))
            )
            u_new = np.dot(u, vh)
            ...
            if i != len(tensor_network.tensors) - 1:
                ...
                target_mpo_adjoint = target_mpo_adjoint @ new_tensor
    ...
    def circuit_from_unitary_layers(
        ...
        """Create a `qiskit.QuantumCircuit` instance from
        the unitary layers.
        ...
        Returns:
            circuit (qiskit.QuantumCircuit): The qiskit circuit.
        """
        circuit = QuantumCircuit(num_qubits)

        for layer in unitary_layers:
            for qubits, unitary_matrix in layer:
                # Note we must update the indices to LSB for qiskit
                circuit.unitary(
                    unitary_matrix,
                    [
                        abs(q - circuit.num_qubits + 1)
    ...
    def __call__(
        ...
        """Approximate a unitary via sweeping.

        Args:
            target_unitary (NDArray[np.complex128]): The unitary we want to approximate.
        ...
        target_mpo = qtn.MatrixProductOperator.from_dense(target_unitary)

        for i in tqdm(range(num_sweeps)):
            circuit_tensor_network = (
                self.get_tensor_network_from_unitary_layers(
            ...
            if log and i % 20 == 0:
                fidelity = hst_fidelity(
                    ...
                    ).reshape(target_unitary.shape),
                    num_qubits,
                )
                print(f"Fidelity: {fidelity}")

            unitary_layers = self.sweep_unitary_layers(
                target_mpo, circuit_tensor_network, unitary_layers
            )
        ...
        if log:
            fidelity = hst_fidelity(
                np.array(Operator(circuit).data).astype(np.complex128),
                target_unitary,
                num_qubits,
            )
