...
from numpy.random import seed as np_seed, rand
...
from ucc.compile import compile
from cirq import Circuit as CirqCircuit
from cirq import CNOT, H, LineQubit, NamedQubit, X
...
def qcnn_circuit(N, seed=12345):
    ...
    qc = QiskitCircuit(N)
    num_layers = int(np_ceil(np_log2(N)))
    i_conv = 0
...
def test_compile_with_mps_pass(N):
    ...
    fidelity = np_abs(
        np_vdot(Statevector(circuit).data, Statevector(compiled_circuit).data)
    )

    assert np_abs(fidelity) > 0.9
...
def test_compile_trivial_state_with_mps_pass():
    ...
    assert np_round(fidelity, decimals=10) == 1.0
...
@mark.parametrize("circuit_function", [qcnn_circuit, random_clifford_circuit])
@mark.parametrize("num_qubits", [6, 7, 8, 9, 10])
@mark.parametrize("seed", [1, 326, 5678, 12345])
...
@mark.parametrize("circuit_function", [qcnn_circuit, random_clifford_circuit])
@mark.parametrize("num_qubits", [6, 7, 8, 9, 10, 15])
@mark.parametrize("seed", [1, 326, 5678, 12345])
