...
from ucc.ssr.circuittransform.inputs.gate_info import gate_depth_cx
...
class DGSwap(DG):
    ...
    def __deepcopy__(self, memo):
        ...
        **({
        ...
        u, v, {
        'qubits': list(data['qubits']),
        **({'depth': data['depth']} if 'depth' in data else {})  # 只在原始数据中存在 'depth' 时才拷贝它
    }
    ...
    def add_swap_node(self):
        ...
        for node in self.nodes:
            ...
            if self.get_node_num_q(node) == 1: continue
            if len(self.get_node_gates(node)) > 1: raise()
...
#### commutation rules
    def find_exchangable_edges(self, 
                               rules=("swap",)):
        '''
    ...
    def find_exchangable_edges_node(self, node,
                                    rules=("swap",)):
        '''
    ...
    def exchangeable(self, node1, node2, 
        ...
        assert self.get_node_num_gate(node1) <= 1 and self.get_node_num_gate(node2) <= 1,\
        ...
        if flag and impose_exchange:
            if rule_impose == "swap":
                flag = self.exchange_swap(node1, node2, 
    ...
    def random_mutation2(self, max_try=None):
        ...
        for _ in range(max_try):
            ...
            if depth_c != depth_ori: break
                
        return count
            
    def exchange_swap(self, node1, node2,
        ...
        if gate1[0] == 'swap':
            ...
            raise(Exception(f"There is no SWAP in nodes {node1} and {node2} but exchange_swap method is imposed."))
        ...
        affected_nodes = set(nx.descendants(self, node1)) | set(nx.descendants(self, node2))
    ...
    def swap_update(self, affected_nodes, node1, node2):
        ...
        for node in affected_nodes:
            ...
            if predecessors:
                ...
                depth_minus = [d - self.node_to_depth[node][0] + 1 for d in self.node_to_depth[node]]
    ...
    def exchange_nodes(self, node1, node2):
        ...
        gate1, gate2 = self.get_node_gates(node1)[0], self.get_node_gates(node2)[0]
        # check 
        if gate1[0] == 'swap' or gate2[0] == 'swap': raise()
        if not self.check_direct_dependency(node1, node2): raise()
    ...
    def exchange(self, node1, node2):
        ...
        if not self.exchangeable(node1, node2): return False
        gate1, gate2 = self.get_node_gates(node1)[0], self.get_node_gates(node2)[0]
        # check commutation
        if gate1[0] == 'swap' or gate2[0] == 'swap':
    ...
    def combine_2q_gates_simple(self, max_q = 5, combine_level=np.inf):
        ...
        while cascade_count > 0 and it_count < combine_level:
            ...
            for node_ori in list(nx.topological_sort(dg_new)):
                ...
                # 2 qubits
                node_new = node_ori
                if not node_new in nodes_groups: 
                    nodes_groups[node_new] = [node_ori] 
                for node_pre_new in qubit_to_node: # to check last node in the qubit
                    if not node_pre_new in dg_new: continue
                    flag_cascade = False
                    if dg_new.check_direct_dependency(node_pre_new, node_new): #current qubits' last node, a
                        # check qubit number
                        ...
                        if len(qubits) <= max_q: flag_cascade = True # can combine
                        # if these two nodes are parallel and share at least one
                        # edge in AG, we still cascade them
                    if not flag_cascade:
                        if dg_new.check_cascadeable([node_pre_new], node_new,max_q_cascade=max_q):
                            for q1 in dg_new.get_node_qubits(node_pre_new):
                                for q2 in dg_new.get_node_qubits(node_new):
                                    if q1 == q2: raise()
                                    if (q1, q2) in edges: flag_cascade = True
                                    if flag_cascade: break
                                if flag_cascade: break
    ...
    def break_all_nodes(self, decompose_swap):
        ...
        dg_new = DGSwap(ag=self.ag, basic_2_q_gate=self.basic_2_q_gate, 
                        cost_func=self.cost_func)
        dg_new.clear_attrs()
        dg_new.from_qiskit_circuit(self.qiskit_circuit(decompose_swap=decompose_swap),
                                   absorb=False)
        return dg_new
    ...
    def find_depth_disables_yq_ver(self,node):
        ...
        node_qubits, node_gates = self.get_node_qubits(node), self.get_node_gates(node)
        ...
        while(qubits_unchecked):
            ...
            [min_q0, min_q1] = [qubit for qubit in node_qubits if gate_placement[min_depths_right][qubit] == process_node]
            depth_beg = min_depths_right + 1
            while(depth_beg < max(qubits_time_slot_max.values())):
                if gate_placement[depth_beg][min_q0] == None and gate_placement[depth_beg][min_q1] == None:
                    qubits_time_slot_min[min_q0] = depth_beg-len(node_gate_to_depth[process_node]) + 1
                    qubits_time_slot_min[min_q1] = depth_beg-len(node_gate_to_depth[process_node]) + 1
                    depth_beg += 1
                else:
                    break
        ...
        while(qubits_unchecked):
            ...
            [max_q0, max_q1] = [qubit for qubit in node_qubits if gate_placement[max_depths_left][qubit] == process_node]
            depth_beg = max_depths_left - 1
            while(depth_beg > min(qubits_time_slot_min.values())):
                if gate_placement[depth_beg][max_q0] == None and gate_placement[depth_beg][max_q1] == None:
                    qubits_time_slot_max[max_q0] = depth_beg + len(node_gate_to_depth[process_node]) - 1
                    qubits_time_slot_max[max_q1] = depth_beg + len(node_gate_to_depth[process_node]) - 1
                    depth_beg -= 1
                else:
                    break
        ...
        for q in node_qubits:
            ...
            for d_reduce in range(time_slot_total):
                ...
                if d_ori < qubits_time_slot_min[q]:
                    ...
                    if gate_placement[d_ori][q] != None and gate_placement[d_ori][q][0] != node:
                        disabled_positions_reduced.append((last_used_pos, q))
                        disabled_positions_ori.append((time_slot_start+last_used_pos, q))
            ...
            for d_reduce in range(time_slot_total-1, -1, -1):
                ...
                if d_ori > qubits_time_slot_max[q]:
                    ...
                    break
        return time_slot_total, (disabled_positions_reduced, disabled_positions_ori),(disabled_left_reduced, disabled_right_reduced)
            
    
    def recompile_cx_node_appro(self, node,predict_depth=None, print_info=False):
        from ucc.ssr.circuittransform.recompile.recompile_sat import recompile_cx_yq,recompile_cx_yq_predict
        cx_list = self.get_node_cx_list(node)
        depth_ori = self.get_node_depth(node)
        if depth_ori == 1: return False, 0, 0, 0
        depth_max, (disabled_positions, _), (dis_left, dis_right) = self.find_depth_disables_yq_ver(node)
        if predict_depth != None:
            cx_list_re, cx_list_depth_re, flag, Sat_times, variable_cout, cnf_total = recompile_cx_yq_predict(cx_list, self.ag, max_depth=depth_max,predict_depth = predict_depth, disabled_pos=disabled_positions, dis_left=dis_left, dis_right=dis_right, max_time=300,
                           print_info=0, min_num_q_compile=16)
        else:
            cx_list_re, cx_list_depth_re, flag, Sat_times, variable_cout, cnf_total = recompile_cx_yq(cx_list, self.ag, max_depth=depth_max, disabled_pos=disabled_positions, dis_left=dis_left, dis_right=dis_right, max_time=300,
                       print_info=0, min_num_q_compile=16)
        gates_new = []
        if len(cx_list_re) > 0:
    ...
    def qiskit_circuit(self, save_to_file=False, add_barrier=False,
                       ...
                       file_name='circuit'):
        dg = self
        return super(DGSwap, dg).qiskit_circuit(save_to_file=save_to_file,
    ...
    def depth_to_node_list(self):
        ...
        while circuit.num_remain_nodes > 0:
            ...
            if self.root in front_nodes: circuit.execute_front_layer()
            ...
            for node in front_nodes:
                for name, qubits, _ in self.get_node_gates(node):
                    ...
                    for _ in range(depth_add):
                        current_depth += 1
                        if not current_depth in self.node_to_depth[node]:
                            self.node_to_depth[node].append(current_depth)
                    # update qubit_to_depth
                    for q in qubits: qubit_to_depth[q] = current_depth
