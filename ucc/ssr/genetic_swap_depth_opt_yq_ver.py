...
from ucc.ssr.circuittransform.inputs.cir_dg_swap_depth_opt import DGSwap, hybridization4
from ucc.ssr.circuittransform.inputs.cir_dg import CreateCircuitFromQASM
...
def equivalence_checking(dg1, dg2, ag):
    ...
    for node in dg.nodes:
        if node == dg.root: continue
        qubits = dg.get_node_qubits(node)
        if len(qubits) == 1: continue
        if len(qubits) == 2:
            if not tuple(qubits) in ag.edges: return False
        if len(qubits) > 2: 
            print("dg1")
            print(qubits)
            raise()
    ...
    for _ in range(max_iter):
        ...
        for species_i in range(num_species):
            if species_i <= mutation_rate * num_species:
                # mutation
                solutions = solutions_pool.pick_solution_random(flag_copy=True, num=1) #original True
                cost, dg_new = solutions[0]
                evolution_count += dg_new.random_mutation2()
            else:
                # hybridization
                solutions = solutions_pool.pick_solution_random(flag_copy=False, num=2)
