...
def and_equal(x, y, z):
    ...
    '''
    if x <= 0 or y <= 0 or z <= 0: raise()
    if x == y or x == z or y == z: raise()
    return [[-1*z, x, y], [-1*z, x, -1*y], [-1*z, -1*x, y], [z, -1*x, -1*y]]
...
def xor_equal(x, y, z):
    ...
    if x <= 0 or y <= 0 or z <= 0: raise()
    if x == y or x == z or y == z: raise()
    return [[-1*z, x, y], [-1*z, -1*x, -1*y], [z, -1*x, y], [z, x, -1*y]]
...
def get_general_cnf(matrix_initial, target_depth, ag_graph,variable_count,qubits_ori_to_reduce, qubits_reduce_to_ori):
    
    #print("qubits reduced to ori ",qubits_reduce_to_ori )
    qubits = list(ag_graph.nodes())
    num_q = len(qubits)
    edges = [(qubits_ori_to_reduce[u],qubits_ori_to_reduce[v]) for (u,v) in ag_graph.edges()]
    ...
    for d in range(target_depth):
        for c in range(num_q):
            ...
            cnf.extend(CardEnc.atmost(lits=variables_, bound=1, encoding=EncType.pairwise))
    # set constraint C3
    for d in range(target_depth):
        for c in range(num_q):
            for t in ag_graph.adj[qubits_reduce_to_ori[c]]:
                ...
                for j in range(num_q):
                    cnf_add.extend(xor_equal(m[d][t][j], m[d][c][j], m[d+1][t][j]))
    ...
    for d in range(target_depth):
        for t in range(num_q):
            for j in range(num_q):
                ...
                cnf_add = CardEnc.equals(lits=variables_, bound=1, encoding=EncType.pairwise).clauses
    ...
    for i in range(num_q):
        for j in range(num_q):
            cnf_add = CardEnc.equals(lits=[int(m[0][i][j])], bound=int(matrix_initial[i][j]), encoding=EncType.pairwise)
            cnf.extend(cnf_add)
...
def get_cnf_variables(matrix_initial, ag, qubits, qubits_ori_to_reduce, qubits_reduce_to_ori, target_depth,disabled=[], min_num_q_compile=np.inf,
                      max_num_variables=np.inf, print_info=False):
    num_q = len(qubits)
    ...
    for edge in ag_sub.edges:
        edges.append((qubits_ori_to_reduce[edge[0]], qubits_ori_to_reduce[edge[1]]))
    t1 = time.time()
    variable_count = 0
    m = np.empty((target_depth+1, num_q, num_q), dtype='int')
    for i in range(m.size):
    ...
    for d in range(target_depth):
        ...
        for q0, q1 in edges:
            ...
            variable_count += 1
            g[d][(q1, q0)] = int(variable_count)
    if variable_count > max_num_variables: raise()
    assump = []
    disable_depths = []
    for d, q in disabled:
        if d > target_depth - 1: continue
        q = qubits_ori_to_reduce[q]
        if not d in disable_depths: disable_depths.append(d)
        for q0, q1 in edges:
            if q in (q0, q1):
    ...
    for d in range(target_depth):
        if d in disable_depths: continue
        cnf_add = []
        for q0, q1 in edges:
            cnf_add.extend([g[d][(q0, q1)], g[d][(q1, q0)]])
        cnf.append(cnf_add)
    ...
    if min_num_q_compile >= num_q:
        ...
        for q in range(num_q):
            cnf.extend(and_equal(id_row[target_depth][q], id_col[target_depth][q], id_row_col_fin[q]))
        cnf_add = CardEnc.atleast(lits=id_row_col_fin, bound=min_num_q_compile,)
        cnf.extend(cnf_add)
    ...
    if True:
        depth_start = min(int(START_DEPTH_RATIO * target_depth + 1), target_depth-1)
    ...
    if print_info:
        print('Time cost for init. {} variables is {}'.format(str(variable_count), str(time.time()-t1)))
    
    return cnf, assump, g, m, variable_count,ag_sub, edges, num_q

def recompile_cx_yq_dis(cx_list, ag, target_depth,disabled_pos = [],dis_left = [], dis_right = [],  solver_name='g41',#'m22', 'g41' 
                    max_num_variables=np.inf, min_num_q_compile=np.inf, max_time=300, # seconds
                    print_info=False):
    ...
    for q_c, q_t in cx_list_r:
        ...
        matrix_initial[q_t] = np.logical_xor(matrix_initial[q_c], matrix_initial[q_t])
    # obtain connectivity information
    ag_sub = nx.subgraph(ag, qubits)
    #nx.draw(ag_sub, with_labels=1)

    cnf, assump, g, m, variable_count, ag_sub, edges, num_q = get_cnf_variables(matrix_initial, ag, qubits, qubits_ori_to_reduce, qubits_reduce_to_ori, target_depth,disabled_pos, min_num_q_compile=min_num_q_compile,
                          max_num_variables=max_num_variables, print_info=print_info)
    ...
    with Solver(name=solver_name, bootstrap_with=cnf) as solver:
        ...
        for dis_offset in range(dis_range+1):
            dis_right_new = [(item[0] + dis_offset, item[1]) for item in new_dis_right if (item[0] + dis_offset) < target_depth ]
            ...
            for d, q in new_dis_pos:
                ...
                for q0, q1 in edges:
                    if q in (q0, q1):
                        assump.append(-1*g[d][(q0, q1)])
                        assump.append(-1*g[d][(q1, q0)])
            flag = solver.solve_limited(assumptions=assump, expect_interrupt=True)
            Sat_times += 1
            if flag == True:
                break
        if print_info:
    ...
    if sat_res == None: return [], [], False, Sat_times, variable_count,num_cnf
...
def recompile_cx_yq(cx_list, ag, min_depth=1, max_depth=None,disabled_pos = [],dis_left = [], dis_right = [],
                    solver_name='g41', max_num_variables=np.inf,min_num_q_compile=np.inf, max_time=300,
                    print_info=False):
    ...
    for d in range(max_depth):
        #print("d+1 is", d+1)
        if d + 1 < min_depth: continue
        cx_list_re, cx_list_depth_re, flag, Sat_times, variable_count, num_cnf = recompile_cx_yq_dis(cx_list, ag, d+1,disabled_pos = disabled_pos,dis_left = dis_left, dis_right = dis_right,  solver_name='g41',#'m22', 'g41' 
                            max_num_variables=max_num_variables, min_num_q_compile=min_num_q_compile, max_time=max_time, # seconds
                            print_info=print_info)
        Sat_total_times += Sat_times
        variable_total += variable_count
        cnf_total += num_cnf
        if len(cx_list_re) > 0: break
        if flag == None: break
    
    return cx_list_re, cx_list_depth_re, flag, Sat_total_times, variable_total, cnf_total


def recompile_cx_yq_predict(cx_list, ag, min_depth=1, max_depth=None, predict_depth = None, disabled_pos = [],dis_left = [], dis_right = [],
                    solver_name='g41', max_num_variables=np.inf,min_num_q_compile=np.inf, max_time=300,
                    print_info=False):
    ...
    for d in range(predict_depth, max_depth+1):
        res = recompile_cx_yq_dis(cx_list, ag, d,disabled_pos = disabled_pos,dis_left = dis_left, dis_right = dis_right,  solver_name='g41',#'m22', 'g41' 
                            max_num_variables=max_num_variables, min_num_q_compile=min_num_q_compile, max_time=max_time, # seconds
                            print_info=print_info)
        cx_list_re, cx_list_depth_re, flag, Sat_times, variable_count, num_cnf  = res
        Sat_total_times += Sat_times
        variable_total += variable_count
        cnf_total += num_cnf
        if len(cx_list_re) > 0: 
    ...
    if d == predict_depth:
        for d in range(predict_depth-1, 1, -1):
            res = recompile_cx_yq_dis(cx_list, ag, d,disabled_pos = disabled_pos,dis_left = dis_left, dis_right = dis_right,  solver_name='g41',#'m22', 'g41' 
                                max_num_variables=max_num_variables, min_num_q_compile=min_num_q_compile, max_time=max_time, # seconds
                                print_info=print_info)
            cx_list_re_new, cx_list_depth_re_new, flag, Sat_times, variable_count, num_cnf  = res
            Sat_total_times += Sat_times
            variable_total += variable_count
            cnf_total += num_cnf
