...
from ucc.ssr.genetic_swap_depth_opt_yq_ver import genetic_opt, equivalence_checking
...
def apply_operations(n, operations):
    ...
    for operation in operations:
        row, col = operation
        M[col] = np.bitwise_xor(M[row].astype(int), M[col].astype(int)).astype(float)
...
def get_neighbors(graph, nodes, num_to_select):
    ...
    while len(selected_neighbors) < num_to_select:
        ...
        round_select = random.sample(neighbors, min(remain_select_num, len(neighbors)))
...
def graph_model_dic():
    ...
    grid_2tri_line1.add_edges_from([(0,1),(1,2),(2,3),(0,3),(0,2),(0,4)])
    graph_model[grid_2tri_line1] = 'q5grid_2tri_line1.pkl'
    
    grid_2tri_line2 = nx.Graph()
    grid_2tri_line2.add_edges_from([(0,1),(1,2),(2,3),(0,3),(0,2),(1,4)])
    graph_model[grid_2tri_line2] = 'q5grid_2tri_line2.pkl'
    
    grid_x_line = nx.Graph()
    grid_x_line.add_edges_from([(0,1),(1,2),(2,3),(0,3),(0,2),(1,3),(0,4)])
...
def get_predict(dg, node):
    ...
    model = joblib.load('/Users/mistywahl/Documents/GitHub/ucc/ucc/ssr/ML_model/'+model_name)
...
def compile_sat_sweeping_num(dg, compile_ratio = 0, compile_num=1, file_name = ''):
    ...
    while node_score:
        ...
        if len(node_score) <= compile_num:
            ...
            compile_node = [k for k, v in sorted(node_score.items(), key=lambda item: item[1], reverse=True)[:compile_num]]
        iterative_times += 1
        iterative_log.append(iterative_times)
        ...
        for node in compile_node:
            ...
            flag, sat_time, variable_cout, num_cnf = dg_new.recompile_cx_node_appro(node, predict_depth=predict_depth, print_info=False)
            Sat_times += sat_time
        ...
        if len(dg_new.swap_nodes) != 0:
            dg_new= genetic_opt(0, dg_new, dg_new.ag, 'depth',  'cx', path=None)
        ...
        time_node_b = time.time()
        node_score,S_times, V_count, cnf_count = get_node_score_predict(dg_new, repeated_node = repeated_node)
        time_node_end = time.time()
        get_node_time_log.append(time_node_end-time_node_b)
        Sat_times += S_times
        total_variable += V_count
    ...
    return dg_new, Sat_times, total_variable, cnf_total, sum(time_compile_log), sum(time_genetic_log), sum(get_node_time_log), iterative_times
...
def run_functions(file, path, ag, basic_2_q_gate, method_cost_func, path_cir):
    ...
    if len(dg.swap_nodes) != 0:
        dg_new = genetic_opt(0, dg, dg.ag, method_cost_func, basic_2_q_gate, path=path)
    
    dg_new = dg.combine_2q_gates_simple(max_q = max_q, combine_level=2)
    
    time2 = time.time()
    
    dg0, sat_times0, total_variable0, cnf_total0, sum_compile_time, sum_genetic_time, sum_predict_time, iterative_times = compile_sat_sweeping_num(dg_new, compile_ratio=0.2)    
    time2_end = time.time()
...
def process_quantum_circuits(input_folder: str, method_AG: list, output_xlsx: str, output_qasm_folder: str):
    """
    ...
    wb = Workbook()
    ws = wb.active
    ws.append(['Name', 'Original Depth', 'Original Gates', 'Transformed Depth', 'Transformed Gates',
               'SAT Times', 'Variable Count', 'CNF Count', 'Total Time', 'iterative_times'])
    
    # Get list of QASM files
    files = [f for f in os.listdir(input_folder) if f.endswith('.qasm')]
    inputs = [(file, input_folder, ag, basic_2_q_gate, method_cost_func, output_qasm_folder) for file in files]
...
if __name__ == "__main__":
    ...
    parser.add_argument( "--path", metavar='S', type=str, help="the path of the input circuits", default="Data/RevLib/Sabre/")
    parser.add_argument("--ag_type", type=str, help="the type of AG graph (sycamore, qgrid, rochester, heron)", default="qgrid")
    parser.add_argument( "--qgrid_params", nargs=2, type=int, metavar=("ROWS", "COLS"),
        help="Specify the grid dimensions if -ag_type is qgrid.", default=[5,4])
    parser.add_argument("--path_excel", type=str, help="The path of output excel data", default="Results/RevLib/Sabre/")
    parser.add_argument("--save_name", type=str, help="The name of excel data", default="qgrid_revlib_sabre")
    parser.add_argument("--path_cir", type=str, help="The path of output circuits", default="Results/RevLib/Sabre/")
    ...
    if args.ag_type == "sycamore":
        ...
        if not args.qgrid_params:
            parser.error("The -qgrid_params must be provided when -ag_type is 'qgrid'.")
        else:
    ...
    process_quantum_circuits(input_folder, method_AG, output_xlsx, output_qasm_folder)
