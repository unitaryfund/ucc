...
from qiskit.transpiler.passes.optimization.collect_2q_blocks import (
    Collect2qBlocks,
)
    ...
    def run(self, dag):
        ...
        for block in blocks:
            if len(block) == 1 and self._check_not_in_basis(
                ...
                max_2q_depth = (
                    20  # If depth > 20, there will be 1q gates to consolidate.
                )
                identity = np.eye(2**unitary.num_qubits)
                if (  # pylint: disable=too-many-boolean-expressions
                    unitary.num_qubits > 2
                ...
                elif self.force_consolidate and np.allclose(
                    identity, unitary.to_matrix()
                ):
                    for node in block:
                        dag.remove_op_node(node)
...
def _block_to_matrix(block, block_index_map):
    ...
    matrix = np.identity(2**block_index_length, dtype=complex)
    for node in block:
        try:
