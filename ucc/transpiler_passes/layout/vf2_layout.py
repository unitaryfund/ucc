...
class VF2Layout(AnalysisPass):
    ...
    def run(self, dag):
        ...
        if self.avg_error_map is None and self.properties is not None:
        ...
        for mapping in mappings:
            ...
            if self.avg_error_map is None:
                layout_score = self._score_layout(im_graph_node_map, im_graph)
    ...
    def _score_layout(self, bit_map, im_graph):
        two_q_gate_penalty = 0
        gate_name_map = get_standard_gate_name_mapping()
        for _, node_index in bit_map.items():
