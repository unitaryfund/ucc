...
from ucc.ssr.circuittransform.inputs.gate_info import supported_gate_names,gate_depth_cx
...
class DG(DiGraph):
    def __init__(self, basic_2_q_gate='cx'):
        ...
        if self.basic_2_q_gate == 'cx':
            ...
            raise(Exception(f'Unsupported 2-qubit gate type {basic_2_q_gate}.'))
...
#### Node and edge related operations
    ...
    def add_gate(self, gate, add_edges=True, update_dg_attrs=True):
        ...
        if max(gate[1])+1 > self.num_q: self.num_q = max(gate[1])+1
        if add_edges:
    ...
    def break_node(self, node):
        ...
        for gate in self.get_node_gates(node):
            ...
            for i, qs in enumerate(qubit_groups):
                if q in qs: 
                    for q in gate[1]:
                        if not q in qs: raise()
                    gate_groups[i].append(gate)
                    break
        pre_nodes = list(self.predecessors(node))
        pre_qubits = [self.get_edge_qubits((pre_node, node)) for pre_node in pre_nodes]
        succ_nodes = list(self.successors(node))
        succ_qubits = [self.get_edge_qubits((node, node_succ)) for node_succ in succ_nodes]
        # delete node
        self.remove_node(node)
        # add nodes
        nodes_add = []
        for gates in gate_groups:
            nodes_add.append(self.add_gates(gates, 
                                            add_edges=False, 
                                            update_dg_attrs=False))
        # add edges
        ## add incoming edges
        for node_pre, qs in zip(pre_nodes, pre_qubits):
            for node_new in nodes_add:
                ...
                edge_qubits = []
                for q in qs:
                    if q in qs_shared: edge_qubits.append(q)
    ...
    def break_node_gates(self, node):
        ...
        nodes_add = []
        for gate in gates:
            nodes_add.append(self.add_gate(gate,
                                           add_edges=False, 
                                           update_dg_attrs=False))
        # add edges
        ## add incoming edges
...
#### File import and export 
    ...
    def from_dg(self, dg, absorb=True):
        ...
        for node in nx.topological_sort(dg):
            ...
            for gate in dg.get_node_gates(node):
                ...
                if not node_add in nodes_add: nodes_add.append(node_add)
    ...
    def qiskit_circuit(self, save_to_file=False, add_barrier=False,
        ...
        if decompose_swap:
            if self.basic_2_q_gate == 'cx':
                from ucc.ssr.circuittransform.inputs.gate_info import gen_swap_via_cx as gen_swap
...
#### Data structure based operations
    ...
    def get_unitary(self, num_q=None, swaps=[]):
        ...
        if num_q == None: num_q = self.num_q
...
#### Gate converter        
    ...
    def schedule(self):
        ...
        for node in nx.topological_sort(self):
            for k, (gate_name, qubits, _) in enumerate(self.get_node_gates(node)):
                depth_ = []
